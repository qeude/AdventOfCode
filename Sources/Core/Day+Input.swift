...
extension Day {
  ...
  public static func rawInput() throws -> String {
    try String(contentsOf: inputFile).trimmingCharacters(in: .newlines)
  }
}
...
extension Day where Input: Parsable {
  public static func transform(raw: String) throws -> Input {
    try .parse(raw: raw)
  }
}

extension Day where Input: Sequence, Input.Element: Parsable {
