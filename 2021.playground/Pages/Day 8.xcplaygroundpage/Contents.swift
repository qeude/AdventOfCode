...
let segmentAndOutput = input
  .split(separator: "\n")
  .map { line -> ([String], [String]) in
    let components = line.components(separatedBy: " | ")
    let segments = components[0].components(separatedBy: " ")
    let output = components[1].components(separatedBy: " ")
    return (segments, output)
  }

let countMapping: [Int: Int] = [
  2: 1,
  3: 7,
  4: 4,
  7: 8,
...
func partOne() -> Int {
  return outputValues
    .flatMap { $0 }
    .filter { countMapping.keys.contains($0.count) }
    .count
}
...
func partTwo() -> Int {
  return segmentAndOutput
    .map { segments, outputs  in
      let decoder = SignalDecoder(patterns: segments)
      return decoder.decode(outputs)
    }
    .reduce(0, +)
}
print(partOne())
